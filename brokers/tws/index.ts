...
export const starter: TStarter = async () => {
  ...
  ib.error.subscribe((error) => {
    logger.add(null, 'TWS', `${error.error.message}`);
  });
};

export const handler: THandler = async (message: string) => {
  const timeStart = performance.now();
  const logOrderId = message.orderId;
  const collection = db.collection(message.channelId);
  while (!ib.isConnected) {
    logger.add(message, 'CONNECTING...');
    ib.disconnect();
    ib.connect(CLIENT_ID);
  ...
  switch (message.type) {
    ...
    case EType.CLOSE: {
      ...
      logger.add(message, 'CLOSE', { previousOrders, openOrders });
      ...
      pendingOrdersId?.every(id => openOrders?.includes(id))) { // true if does not close for pending orders (need close manually)
        logger.add(message, 'CLOSE BEFORE LIMIT EXECUTION');
  ...
  logger.add(message, 'HANDLE EXECUTION', (timeFinish - timeStart).toFixed(2));
};
